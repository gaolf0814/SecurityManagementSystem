...
class OpcClient:
    def __init__(self, opc_url: str, nodes_path_dict: dict):
        self.opc_url = opc_url
        self.nodes_path_dict = nodes_path_dict
        self.client = None
        self.nodes_dict = {}
        self.connect()
    ...
    def connect(self):
        ...
        try:
            ...
            for name in self.nodes_path_dict.keys():
                if self.nodes_path_dict[name] is not None:
                    self.nodes_dict[name] = self.client.get_node(self.nodes_path_dict[name])
        except timeout:
    ...
    def stop_it(self, name):
        try:
            if self.just_reconnected:  # 如果重新连接后成功读取节点信息，则标志置 False
                self.just_reconnected = False
            self.set_value(name, self.nodes_dict[name], True)  # 停机
