...
class Visualize:
    ...
    def draw(self, frames_dict, preds_dict, judgements_dict, fps):
        ...
        for name in preds_dict.keys():
            ...
            if pred is not None:
                for i, xyxy in enumerate(pred):
                    x1, y1, x2, y2 = xyxy
                    # label = 'person' + str(cls[i])
                    box_area = (x2 - x1) * (y2 - y1)
                    # 过滤掉过大和过小的识别框
                    if min_object_bbox_area_dict[name] <= box_area <= max_object_bbox_area_dict[name]:
                        plot_one_box((x1, y1, x2, y2), img, color=(225, 225, 0))

            if judgement:
                img = cv2.putText(img, text='ATTENTION!!!', org=(10, img.shape[0] - 10),
                                  fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1.2, color=(0, 0, 255), thickness=2)
            else:
                img = cv2.putText(img, text='SAFE WORKING', org=(10, img.shape[0] - 10),
                                  fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1.2, color=(0, 255, 0), thickness=2)
